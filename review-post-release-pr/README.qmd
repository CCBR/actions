...
3. **Only version/date bumps were made** – every change in those files must
   be a version or date bump consistent with the release found in condition
   1. A readme file's suggested citation snippet may also be re-rendered by
   the `auto-format` action in response to the version/date bump; that type
   of change is accepted the same way.
