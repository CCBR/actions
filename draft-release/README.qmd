...
When `version-tag` is provided, the action uses that version even if there are
no new commits or no conventional commits that determine a version bump. When
`version-tag` is omitted, the commit history must determine the next version.
