...
- New action: `review-pre-commit-pr` to automatically review pre-commit.ci autoupdate PRs and approve them when only `rev:` version bumps are present, or request a human reviewer otherwise. (#189, @kelly-sovacool, @copilot)
  ...
  - Fix `review-pre-commit-pr` to auto-approve version changes where a moving `vX.Y` tag replaces a more specific `vX.Y.Z` tag pointing at the same commit, instead of requiring human review as a false-positive downgrade. (#219, @kelly-sovacool, @copilot)
