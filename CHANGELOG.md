...
- New action: `review-post-release-pr` to automatically review post-release cleanup PRs and approve them (approving pending workflow runs and enabling auto-merge) when only version/date bumps matching an actual release tag are present, or request a human reviewer otherwise. (#227, @kelly-sovacool, @copilot)
...
- Create `ccbr_actions` docker image and show how to use it in example workflows. (#190, @kelly-sovacool, @copilot)
  - Resolve `GITHUB_WORKSPACE` at runtime and register it as a safe Git directory in container jobs, working around actions/runner#2058 and actions/checkout#1169. (#214, @kelly-sovacool, @copilot)
  - Default composite actions to Python 3.14, matching the `ccbr_actions` container runtime. (#214, @kelly-sovacool, @copilot)
- Allow `draft-release` to handle first-release repos and fail fast on required files. (#179, @kelly-sovacool, @copilot)
- New action: `review-pre-commit-pr` to automatically review pre-commit.ci autoupdate PRs and approve them when only `rev:` version bumps are present, or request a human reviewer otherwise. (#189, @kelly-sovacool, @copilot)
  ...
  - Handle case where a moving `vX.Y` tag replaces a more specific `vX.Y.Z` tag pointing at the same commit. (#219, @kelly-sovacool, @copilot)
